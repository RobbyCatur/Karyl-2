...
handler.all = async function (m) {
    ...
    if ((m.mtype === 'groupInviteMessage' || m.text.startsWith('https://chat') || m.text.startsWith('Buka tautan ini')) && !m.isBaileys && !m.isGroup) {
        this.sendButton(m.chat, `┌「 *Invite bot ke group* 」
├ 1 Hari Free Trial
├ 30 Hari Rp.5000
├ Berminat? Silahkan tap button dan hubungi owner saya
└────
`.trim(), wm, 'Pemilik Bot', '.owner', m)
    ...
    if (/^bot$/i.test(m.text)) {
        await this.reply(m.chat, !(m.isGroup || m.isPrems) && group ? 'hanya grup' : isBanned ? 'chat banned' : banned ? 'user banned' : 'yo', m)
    }
